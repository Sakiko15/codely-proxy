...
路由集中在 `internal/webui/server.go` 的 `Routes()`：`/v1/*` → 代理 Handler；`/api/*` → WebUI 管理端（独立的 cookie 会话鉴权，与客户端 API key 是两套域）；`/healthz`；`GET /{$}` 单页 shell + `GET /web/*` 静态资源（`static.go` 预载白名单 + 显式 MIME + sha256 ETag 协商 + CSP self-only）。admin API：账号 delete/switch/import/export、设备码登录 start/status/cancel、balancer/security 的 config+status、security keys 增删/明文取回（`keys/add` 空 key=自动生成；`keys/reveal` 按需取明文，status 常态只回脱敏）、quota 查询、`GET /api/logs`（proxy 请求环形缓冲快照，`since` 游标增量）、`GET /api/models` + `POST /api/models/probe`（后端探测，显式管理员动作，前端确认框明示计费成本，**绝不自动触发**）——**无轮换/驱逐端点**。前端是多文件 ES modules（零依赖零构建链，`web/index.html` shell + `web/assets/*` + `web/pages/*` 六页，hash 路由），针刺测试（`webui_test.go`）以 `webSource` 聚合预载资源跨文件钉死前端修复契约，改前端后若测试挂优先核对契约语义而非删针刺。
