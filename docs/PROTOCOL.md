...
> **2026-08-26 补充（重要）**：同一 400 文案**不止 UA 一种触发**。网关还会扫描请求体 system 文本，命中以下内容同样返回该 400（`/v1/messages` 路径实测，大小写不敏感）：
> 1. `x-anthropic-billing-header` —— Claude Code v2.1.246 起把计费头文本嵌入 system 提示，被判为「请求头注入」；
> 2. `you are claude code` —— 被判为「Claude Code 身份冒充」。
>
> 桥已在 `transformBody`（codely-proxy.js）中做无害清洗（剥离计费头行、身份短语改写为通用说法），Claude Code 直连桥不受影响；若客户端绕过桥直连网关，携带上述内容的请求会被 400 拒绝。
...
| 400 | `欢迎使用Codely, 访问 ...` | UA 校验未过 **或** system 文本含违禁内容（`x-anthropic-billing-header` / `you are claude code`）→ 见 §2.2 |
